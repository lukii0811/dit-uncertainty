"""Paired SiT/flow-matching inference for controlled TREAD experiments.

The same seed-class manifest can be reused for the dense baseline, routing at
every solver step, and routing at one selected step. Paired mode stores Xt
latents and model-predicted X0 latents/images. FID mode stores final PNGs only.
"""
...
def resolve_save_policy(args):
    ...
    policy = {
        "xt": args.save_xt,
        "x0_latents": args.save_x0_latents,
        "x0_images": args.save_x0_images,
...
def resolve_routing(args):
    ...
    if not 0 <= args.routing_step < args.num_steps:
        raise ValueError(
            f"routing_step must be in [0, {args.num_steps - 1}]"
        )
    return int(args.routing_step), mask_ratio, ratio_source


def _resolve_path_from_script(value):
    """Resolve relative model paths against this script, not the launch CWD."""
...
def _find_diffusers_vae_directory(root):
    """Find the actual Diffusers AutoencoderKL directory below ``root``."""
    ...
    def is_vae_config(config_path):
        ...
        return (
            "autoencoder" in class_name
            or (
                "latent_channels" in config
                and "block_out_channels" in config
            )
        )

    direct_config = root / "config.json"
    if direct_config.is_file() and is_vae_config(direct_config):
        return root
    ...
    if len(candidates) > 1:
        ...
        raise RuntimeError(
            "Multiple Diffusers VAE directories were found. Pass the exact "
            f"directory with --vae-model:\n  {rendered}"
        )
    ...
    raise FileNotFoundError(
        "No Diffusers AutoencoderKL config.json was found under "
        f"{root}. Files seen: {files}"
    )


def resolve_vae_reference(args):
    """Resolve a Hugging Face ID, ModelScope snapshot, or local directory."""
    if args.vae_source == "local":
        requested_path = _resolve_path_from_script(args.vae_model)
        path = _find_diffusers_vae_directory(requested_path)
    ...
    if args.vae_source == "modelscope":
        ...
        try:
            resolved = snapshot_download(
                args.modelscope_vae_id, **download_kwargs
            )
        except TypeError as error:
            # Older ModelScope releases may not support local_dir. Fall back to
            # cache_dir while preserving an explicit destination when possible.
            if "local_dir" not in download_kwargs:
                raise
            resolved = snapshot_download(
                args.modelscope_vae_id,
                cache_dir=download_kwargs["local_dir"],
            )
...
def load_vae(args, device, model_dtype):
    try:
        ...
        raise ImportError(
            "Decoded images were requested, but diffusers is not installed"
        ) from error
    vae_dtype = (
    ...
    vae_reference, local_only, source_report = resolve_vae_reference(args)
    vae = AutoencoderKL.from_pretrained(
        vae_reference,
        torch_dtype=vae_dtype,
        local_files_only=local_only,
    ).to(device)
    source_report["dtype"] = str(vae_dtype)
    return vae.eval(), vae_dtype, source_report
...
def decode_x0_predictions(
    ...
    steps, batch = x0_predictions.shape[:2]
    flat = x0_predictions.reshape(
        steps * batch, *x0_predictions.shape[2:]
    )
    decoded = decode_latents_to_uint8(
        vae,
        flat,
        decode_batch_size,
        vae_dtype,
    ...
    decoded = decoded.reshape(steps, batch, *decoded.shape[1:])
    return decoded.permute(1, 0, 2, 3, 4).contiguous()
...
def save_png_batch(images, pairs, directory):
    ...
    for image, pair in zip(images, pairs):
        ...
        Image.fromarray(array).save(
            directory / f"{pair['sample_id']:06d}.png"
        )


def batch_is_complete(output_dir, pairs, policy):
    start_id = pairs[0]["sample_id"]
    end_id = pairs[-1]["sample_id"]
    tensor_path = (
        output_dir
        / "tensor_shards"
        / f"shard_{start_id:06d}_{end_id:06d}.pt"
    )
    tensor_required = any(
        policy[key]
        for key in (
    ...
    tensors_done = tensor_path.exists() if tensor_required else True
    pngs_done = (
        all(
    ...
    return tensors_done and pngs_done
...
def write_run_manifest(output_dir, manifest, resume):
    ...
    if path.exists():
        ...
        if existing != manifest:
            raise ValueError(
                f"Existing run manifest at {path} does not match this run"
            )
        if not resume:
...
def save_tensor_shard(
    ...
    payload = {
        "format": "tread-sit-paired-inference-shard-v1",
        ...
        "x0_prediction_times": (
            result["x0_prediction_times"].cpu()
            if result["x0_prediction_times"] is not None
            else None
        ),
    }
    if policy["xt"]:
    ...
    if policy["x0_latents"]:
        payload["x0_predictions"] = result["x0_predictions"].permute(
            1, 0, 2, 3, 4
        ).cpu()
    if policy["x0_images"]:
        payload["x0_decoded_uint8"] = x0_images
    ...
    shard_dir = output_dir / "tensor_shards"
    shard_dir.mkdir(parents=True, exist_ok=True)
    start_id = pairs[0]["sample_id"]
    end_id = pairs[-1]["sample_id"]
    torch.save(
        payload,
        shard_dir / f"shard_{start_id:06d}_{end_id:06d}.pt",
    )
...
def run(args):
    ...
    write_json(output_dir / "checkpoint_load_report.json", checkpoint_report)
    ...
    for batch_start in range(0, len(pairs), args.batch_size):
        ...
        if args.resume and batch_is_complete(output_dir, batch_pairs, policy):
            print(
                "Skipping completed batch beginning with sample "
                f"{batch_pairs[0]['sample_id']}"
            )
            continue

        latents, labels = make_initial_latents(
        ...
        result = sit_sampler_with_routing(
            ...
            mask_ratio=mask_ratio,
            mask_seed=args.mask_seed,
            mask_sample_seeds=make_mask_sample_seeds(
        ...
        x0_images = None
        if policy["x0_images"]:
            x0_images = decode_x0_predictions(
                vae,
...
def build_parser():
    ...
    parser.add_argument("--checkpoint", default="./parameters/0400000.pt")
    ...
    ratio_group.add_argument(
        ...
        type=float,
        default=None,
        help="Fraction of tokens masked; 0 means all tokens are kept",
    )
    ratio_group.add_argument(
        "--selection-ratio",
        type=float,
        default=None,
        help="Fraction of tokens kept; Mentor default is 0.5",
    )
    ...
    parser.add_argument(
        ...
        default="local",
        help="Local is the default; relative paths are resolved beside this script",
    )
    parser.add_argument(
        "--vae-model",
        # default="stabilityai/sd-vae-ft-mse",
        default="/data1/huanghaitao/TREAD/parameters/sdvae/sd-vae-ft-mse/",
        help="Hugging Face model ID, or a directory with --vae-source local",
    )
    parser.add_argument(
        "--modelscope-vae-id",
        default="q2792046875/sd-vae-ft-mse",
    )
    parser.add_argument("--modelscope-cache-dir", default=None)
    parser.add_argument("--modelscope-local-dir", default=None)
    parser.add_argument(
        "--vae-dtype",
        choices=["model", *DTYPES.keys()],
        default="model",
    )
    ...
    parser.add_argument(
        "--save-mode",
        choices=["paired", "fid", "custom"],
        default="paired",
    )
    parser.add_argument("--save-xt", action="store_true")
    parser.add_argument("--save-x0-latents", action="store_true")
    parser.add_argument("--save-x0-images", action="store_true")
