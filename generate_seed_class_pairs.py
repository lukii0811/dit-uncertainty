...
def generate_pairs(
    ...
    total = len(class_ids) * samples_per_class
    if seed_max < seed_min or seed_max - seed_min + 1 < total:
        raise ValueError("The seed range does not contain enough unique seeds")
    rng = random.Random(noise_master_seed)
    seeds = iter(rng.sample(range(seed_min, seed_max + 1), total))
    pairs = []
...
def build_parser():
    ...
    group.add_argument("--classes", help="IDs/ranges, e.g. 0,7,42,100-112")
    group.add_argument("--class-file")
    group.add_argument("--num-selected-classes", type=int)
    parser.add_argument(
        "--class-selection", choices=["random", "first"], default="random"
    )
